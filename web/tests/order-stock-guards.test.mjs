...
test('catalog + actual order API + SQL stock transaction regression', async (t) => {
  ...
  try {
    ...
    await db.exec(sql('20260915123000_order_stock_guards'));
    ...
    const sdk = {
      ...
      rpc(name,args) {
        ...
        return { async single() {
          try { return { data: await row('select * from create_order_with_stock($1,$2,$3,$4,$5,$6,$7,$8,$9)',[args.p_table_id,args.p_session_token,args.p_order_note,args.p_total_amount,JSON.stringify(args.p_items),JSON.stringify(args.p_required_items),args.p_dining_type,args.p_request_id??null,args.p_request_fingerprint??null]),error:null }; }
    ...
    await t.test('retry returns original order, changed payload is rejected, stock deducts once', async () => {
      const requestId=randomUUID(),sessionToken=randomUUID();
      const before=await query('select id,stock_quantity from ingredients order by id');
      const first=await order([opt('ไก่'),opt('ไข่ดาว'),opt('เพิ่มข้าว',2)],{requestId,sessionToken});assert.equal(first.status,201,JSON.stringify(first.body));assert.equal(first.body.order.stock_deducted,true);
      const deducted=await query('select id,stock_quantity from ingredients order by id');
      const again=await order([opt('ไก่'),opt('ไข่ดาว'),opt('เพิ่มข้าว',2)],{requestId,sessionToken});assert.equal(again.status,200);assert.equal(again.body.order.id,first.body.order.id);assert.deepEqual(await query('select id,stock_quantity from ingredients order by id'),deducted);
      assert.equal((await order([opt('ทะเล')],{requestId,sessionToken})).status,409);
      await query("select cancel_order_and_restore_stock($1,'confirmed')",[first.body.order.id]);assert.deepEqual(await query('select id,stock_quantity from ingredients order by id'),before);
    });
    await t.test('empty base or option recipe fails closed without creating orders', async () => {
      const before=await row('select count(*)::int n from orders');
      const recipe=await query('select * from menu_ingredients where menu_id=$1',[menu.id]);await query('delete from menu_ingredients where menu_id=$1',[menu.id]);assert.equal((await order([opt('ไก่')])).status,409);
      for(const r of recipe)await query('insert into menu_ingredients values($1,$2,$3)',[r.menu_id,r.ingredient_id,r.quantity_required]);
      const egg=await row("select * from addon_ingredients where addon_id=(select id from addons where name='ไข่ดาว')");await query('delete from addon_ingredients where addon_id=$1',[egg.addon_id]);assert.equal((await order([opt('ไก่'),opt('ไข่ดาว')])).status,409);
      await query('insert into addon_ingredients values($1,$2,$3)',[egg.addon_id,egg.ingredient_id,egg.quantity_required]);assert.deepEqual(await row('select count(*)::int n from orders'),before);
    });

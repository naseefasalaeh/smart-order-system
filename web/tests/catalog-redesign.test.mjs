...
test('new migrations have unique timestamps and explicit transactions', () => {
  ...
  for (const f of files.filter((f) => f.startsWith('20260914'))) {
...
test('catalog + actual order API + SQL stock transaction regression', async (t) => {
  ...
  try {
    ...
    await t.test('seed preserves history, referenced recipes and balances; deletes only unused demos', async () => {
      ...
      assert.equal((await row('select count(*)::int n from addons')).n,4);
      ...
      assert.deepEqual((await query('select name from effective_menu_options where menu_id=$1 and addon_id is not null order by name',[tm.id])).map((r)=>r.name).sort(),['เพิ่มข้าว','เพิ่มเนื้อ'].sort());
    });
    ...
    await t.test('central addon edits affect multiple menus, snapshots stay unchanged, unchecked links are rejected', async () => {
      ...
      const a=await row('select save_addon(null,$1,10,true,3,$2) id',['ไข่ต้ม',JSON.stringify([{ingredient_id:egg.id,quantity_required:1}])]);
      ...
      await query('select save_addon($1,$2,15,true,3,$3)',[a.id,'ไข่ต้ม',JSON.stringify([{ingredient_id:egg.id,quantity_required:2}])]);

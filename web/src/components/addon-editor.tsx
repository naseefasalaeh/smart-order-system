  ...
  return <form action={async (form) => {
    ...
    try {
      ...
      if (result.error) setMessage(result.error);
        ...
        onSaved?.(result.addon); router.refresh();
      }
    ...
    <fieldset className="space-y-3"><legend className="font-bold">สูตรต่อ 1 ตัวเลือก</legend>
      {rows.map((row) => <div key={row.key} className="flex items-end gap-2">
        ...
        <button type="button" disabled={pending} className="p-2 text-red-700" onClick={() => setRows(rows.filter((r) => r.key !== row.key))}>นำออก</button>
      </div>)}
      <button type="button" className="text-orange-700" onClick={() => setRows([...rows, { key: key.current++, ingredient_id: 0, quantity_required: 1 }])}>+ เพิ่มวัตถุดิบในสูตร</button>
    </fieldset>
    ...
    <button disabled={pending} className="rounded-xl bg-orange-600 px-4 py-3 font-bold text-white disabled:opacity-50">{pending ? "กำลังบันทึก..." : "บันทึกตัวเลือกเสริม"}</button>
  </form>;
}

...
export async function saveTable(form: FormData) {
  ...
  if ((id !== null && (!Number.isSafeInteger(id) || id <= 0)) || !number || number.length > 40) {
    return { error: "ข้อมูลโต๊ะไม่ถูกต้อง" };
  }
  ...
  if (error) {
    ...
    return { error: error.code === "23505" || error.message.includes("TABLE_NUMBER_RESERVED") ? "หมายเลขโต๊ะนี้ถูกใช้แล้ว" : "บันทึกโต๊ะไม่สำเร็จ กรุณาลองใหม่" };
  }

  ...
  return (
    <div className="mt-6 space-y-5">
      <details className="rounded-xl border border-orange-200 bg-orange-50 p-5">
        ...
        <ActionForm action={saveOptionGroup} className="mt-5">
          <input type="hidden" name="menu_id" value={menuId} />
          <GroupFields />
          <p className="mt-3 text-xs text-zinc-500">กลุ่มบังคับต้องสร้างแบบปิดไว้ก่อน เพิ่มตัวเลือก แล้วจึงเปิดใช้งาน</p>
          <SubmitButton className="mt-4 rounded-lg bg-orange-500 px-4 py-2 font-semibold text-white">เพิ่มกลุ่ม</SubmitButton>
        </ActionForm>
      </details>
        ...
        return (
          <section key={group.id} className="rounded-2xl border border-zinc-200 p-5">
            <details>
              ...
              <ActionForm action={saveOptionGroup} className="mt-5 rounded-xl bg-zinc-50 p-4">
                <input type="hidden" name="menu_id" value={menuId} />
                <GroupFields group={group} />
                <SubmitButton className="mt-4 rounded-lg bg-zinc-900 px-4 py-2 font-semibold text-white">บันทึกกลุ่ม</SubmitButton>
              </ActionForm>
            </details>
            ...
            <div className="mt-5 space-y-4 border-t border-zinc-200 pt-5">
              {groupOptions.map((option) => {
                ...
                return (
                  <details key={option.id} className="rounded-xl border border-zinc-200 p-4">
                    ...
                    <ActionForm action={saveMenuOption} className="mt-4 grid gap-3 md:grid-cols-2">
                      <input type="hidden" name="menu_id" value={menuId} />
                      ...
                      <SubmitButton className="rounded-lg bg-zinc-900 px-4 py-2 font-semibold text-white">บันทึกตัวเลือก</SubmitButton>
                    </ActionForm>
                    ...
                    <div className="mt-5 border-t pt-4">
                          ...
                          return (
                            <div key={recipe.ingredient_id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-orange-50 p-3 text-sm">
                              ...
                              <div className="flex gap-2">
                                <ActionForm action={saveOptionIngredient} className="flex gap-2">
                                  ...
                                  <SubmitButton className="font-semibold text-zinc-700">อัปเดต</SubmitButton>
                                </ActionForm>
                                <ActionForm action={removeOptionIngredient}>
                                  <input type="hidden" name="menu_id" value={menuId} /><input type="hidden" name="option_id" value={option.id} /><input type="hidden" name="ingredient_id" value={recipe.ingredient_id} />
                                  <SubmitButton className="font-semibold text-red-600">ลบออก</SubmitButton>
                                </ActionForm>
                              </div>
                            </div>
                          );
                      ...
                      <ActionForm action={saveOptionIngredient} className="mt-3 grid gap-2 sm:grid-cols-[1fr_130px_auto]">
                        ...
                        <SubmitButton className="rounded-lg bg-orange-500 px-4 py-2 font-semibold text-white">เพิ่มในสูตร</SubmitButton>
                      </ActionForm>
                    </div>
                  </details>
                );
              ...
              <details className="rounded-xl border border-dashed border-zinc-300 p-4">
                ...
                <ActionForm action={saveMenuOption} className="mt-4 grid gap-3 md:grid-cols-2">
                  ...
                  <SubmitButton className="rounded-lg bg-orange-500 px-4 py-2 font-semibold text-white">เพิ่มตัวเลือก</SubmitButton>
                </ActionForm>
